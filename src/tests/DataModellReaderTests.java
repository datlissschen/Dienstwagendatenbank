...
public class DataModellReaderTests {
    ...
    public void testCorrectImportOfAllModels() throws IOException {
        ...
        assertEquals(1, Data.fahrerListe.size(), "project.Fahrer should be imported");
        assertEquals(1, Data.dienstwagenListe.size(), "Dienstwagen should be imported");
        assertEquals(1, Data.fahrtenListe.size(), "project.Fahrt should be imported");

        Fahrer f = Data.fahrerListe.getFirst();
        assertEquals("F001", f.getFahrerID());
        assertEquals("Max", f.getVorname());

        Dienstwagen v = Data.dienstwagenListe.getFirst();
        assertEquals("Audi", v.getHersteller());

        Fahrt ft = Data.fahrtenListe.getFirst();
